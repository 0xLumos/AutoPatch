#!/usr/bin/env bash
###############################################################################
#  AutoPatch — Full Experiment Runner v2 (FIXED)
#
#  Fixes from v1:
#    1. AutoPatch now maps to -alpine/-slim variants (fewest CVEs)
#    2. Copa uses direct buildkit address, false positive detection added
#    3. Docker Scout parsing fixed for actual output format
#    4. Reduction % calculation fixed (was showing 0% for 18→0)
#    5. Trivy re-scan failure now correctly marks Copa as FAILED
#    6. Distroless/scratch images handled correctly (not remapped to ubuntu)
#
#  Usage:
#    chmod +x run_full_experiment_v2.sh
#    sudo bash run_full_experiment_v2.sh 2>&1 | tee experiment_v2.log
#
#  To re-run fresh (clearing v1 data):
#    sudo CLEAN=1 bash run_full_experiment_v2.sh 2>&1 | tee experiment_v2.log
###############################################################################
set -euo pipefail
...
FIGURES_DIR="${WORK_DIR}/figures_v2"
PROGRESS_FILE="${WORK_DIR}/progress_v2.txt"
...
echo "Experiment v2 started at $(date)" > "$PROGRESS_FILE"
...
# Check if everything from v1 is already installed
MISSING=0
for tool in docker trivy cosign copa; do
...
if [ "$MISSING" = "1" ]; then
    echo ""
    echo "Some tools are missing. Run v1 first to install them:"
    echo "  sudo bash run_full_experiment.sh"
    echo ""
    echo "Or install manually:"
    echo "  Docker: curl -fsSL https://get.docker.com | sh"
    echo "  Trivy:  see https://aquasecurity.github.io/trivy"
    echo "  Copa:   see https://github.com/project-copacetic/copacetic"
    echo ""
    # Continue anyway — missing tools will just fail gracefully
    warn "Continuing with missing tools — those strategies will record failures"
fi
...
# Trivy DB
step "Ensuring Trivy DB is current..."
trivy image --download-db-only 2>/dev/null && ok "Trivy DB ready" || warn "Trivy DB update failed"

# Copa buildkit
COPA_BK=""
if command -v copa &>/dev/null; then
    # Ensure buildkit container exists
    if ! docker buildx ls 2>/dev/null | grep -q copabuildkit; then
        docker buildx create --name copabuildkit --use --bootstrap 2>/dev/null || true
    fi
    COPA_BK=$(docker ps --filter "name=buildx_buildkit_copabuildkit" --format '{{.Names}}' 2>/dev/null | head -1)
    if [ -n "$COPA_BK" ]; then
        ok "Copa buildkit container: $COPA_BK"
    else
        warn "Copa buildkit container not running"
    fi
fi

# Python check
python3 -c "import scipy, matplotlib, numpy, json; print('Python deps OK')" \
    && ok "Python dependencies verified" \
    || { warn "Installing Python deps..."; pip3 install --break-system-packages scipy matplotlib numpy seaborn 2>/dev/null || pip3 install scipy matplotlib numpy seaborn; }

# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║  PHASE 2: ENSURE REPO AND DOCKERFILES EXIST                            ║
# ╚═══════════════════════════════════════════════════════════════════════════╝
...
# Count existing Dockerfiles
DF_COUNT=$(ls dockerfiles/Dockerfile.* 2>/dev/null | wc -l)
if [ "$DF_COUNT" -lt 50 ]; then
    warn "Only $DF_COUNT Dockerfiles found — running v1 first to generate dataset"
    echo "Run: sudo bash experiments/run_full_experiment.sh"
    echo "Then re-run this v2 script."
    # Generate inline anyway
    step "Generating Dockerfiles..."
    python3 experiments/generate_dataset.py 2>/dev/null || true
fi
DF_COUNT=$(ls dockerfiles/Dockerfile.* 2>/dev/null | wc -l)
ok "Dataset ready: $DF_COUNT Dockerfiles"

# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║  PHASE 3: RUN CORRECTED EXPERIMENT                                     ║
# ╚═══════════════════════════════════════════════════════════════════════════╝
phase 3 "Running 5-strategy experiment (v2 — fixed)"

mkdir -p "$RESULTS_DIR"/{scans,sboms,patched}
export DOCKER_CLI_HINTS=false

# ── Write the corrected Python experiment runner ─────────────────────────
cat > "${WORK_DIR}/run_experiment_v2.py" << 'PYEOF'
#!/usr/bin/env python3
"""
AutoPatch 5-strategy experiment runner v2.

FIXES:
  1. choose_base() now maps to -alpine variants (fewest CVEs)
  2. Copa: passes buildkit addr via -a flag; detects Trivy re-scan failures
  3. Scout: fixed output parsing for actual docker scout output
  4. Reduction %: fixed division logic
  5. Distroless/scratch: not remapped to wrong OS families
  6. Images with 0 baseline vulns: correctly handled
"""
...
def trivy_scan(image, out_file):
    """Run Trivy scan. Returns parsed JSON or None on failure."""
    code, _, err = run([
...
def extract_vulns(scan):
    """Extract per-severity counts and CVE IDs. Returns (counts, cve_list)."""
...
# ═══════════════════════════════════════════════════════════════════════
# FIXED v3: Base image chooser — preserve language version, change OS to alpine
# ═══════════════════════════════════════════════════════════════════════
def _extract_ver(tag, default=""):
    """Extract major.minor version from a tag like 'python:3.8-bookworm'."""
...
def choose_base(family, orig_base=""):
    """
    Choose the best replacement base image to MINIMIZE vulnerabilities.

    v3 STRATEGY (fixes v2 regression where upgrading language versions
    INCREASED CVEs because newer runtimes have more catalogued vulns):

    - Language runtimes → keep SAME major.minor version, switch OS to alpine
      This isolates OS-level improvement without language version risk.
    - Pure OS images → upgrade to latest stable of same family
    - Infrastructure services → upgrade to latest + alpine
    - Distroless → keep as distroless (do NOT remap to ubuntu)
    - Scratch → keep as scratch
    """
    o = orig_base.lower()

    # ── Language runtimes → SAME version, alpine OS ──────────────────
    # Key insight: upgrading python:3.8 → python:3.13 or golang:1.16 → 1.23
    # can INCREASE CVEs because newer runtimes have more catalogued vulns.
    # Instead, just switch the OS layer to alpine (OS-level patching).

    if "python" in o:
        ver = _extract_ver(orig_base, "3.12")
        # Python alpine images exist for all maintained versions
    ...
    if "php" in o:
        ...
        if "apache" in o: return f"php:{ver}-apache"  # no alpine for apache
        if "fpm" in o:    return f"php:{ver}-fpm-alpine"
        return f"php:{ver}-alpine"

    if "temurin" in o or "adoptopenjdk" in o:
        # Keep same Java major version, switch to JRE alpine (smaller than JDK)
    ...
    if "maven" in o:
        # Maven: keep same Maven major, use temurin alpine
        return "maven:3.9-eclipse-temurin-21-alpine"

    # ── Web servers / infrastructure → latest + alpine ────────────────
    # These are safe to upgrade because they're services, not SDKs.
    ...
    # ── Databases → latest stable ─────────────────────────────────────
    if "redis" in o:    return "redis:7-alpine"
    if "postgres" in o: return "postgres:17-alpine"
    if "mysql" in o:    return "mysql:8.4"      # no alpine variant
    if "mongo" in o:    return "mongo:7"         # no alpine variant
    if "mariadb" in o:  return "mariadb:11"      # no alpine variant
    ...
    # ── Pure OS images: upgrade within same family ────────────────────
    f = family.lower()
    MAP = {
        "alpine":     "alpine:3.21",
...
def rewrite_autopatch(text, sbom_data):
    """SBOM-driven rewrite: replace each FROM with lowest-vuln equivalent."""
    ...
    for line in text.splitlines():
        ...
        if s.upper().startswith("FROM "):
            ...
            if len(parts)==2:
                ...
                base_part = m.group(1) if m else rest
                alias_part = m.group(2) if m else ""
                # Don't rewrite scratch or internal stage references
                if base_part.lower() == "scratch":
                    lines.append(line); continue
                if not (":" in base_part or "/" in base_part or "." in base_part):
                    lines.append(line); continue  # internal alias like "FROM builder"
...
class Experiment:
    def __init__(self, df_dir, out_dir, strategies, max_images=0, copa_bk=""):
        ...
        self.results = []
        self.tmp = Path(tempfile.mkdtemp(prefix="apv2_"))
        # Crash recovery
    ...
    def safe_tag(self, name, strat):
        return f"v2-{re.sub(r'[^a-z0-9._-]','-',name.lower())}-{strat}"

    def vr(self, before, after):
        """Vulnerability reduction %. Positive = good (fewer vulns)."""
        if before == 0:
            if after == 0: return 0.0
            return -100.0  # new vulns from nothing
    ...
    def process(self, df_path):
        ...
        # Extract original base image name from first FROM line
        orig_base = ""
        for line in text.splitlines():
        ...
        if not ok:
            log.warning(f"  BUILD FAILED ({berr})")
        ...
        if "scan-only" in self.strategies and (name,sn) not in self.done:
            results.append(R(image_name=name,strategy=sn,build_success=True,
                             ...
                             cves_before=cves_b,cves_after=cves_b,base_before=orig_base))
        ...
        if "naive" in self.strategies and (name,sn) not in self.done:
            ...
            r = R(image_name=name,strategy=sn,vulns_before_total=total_b,
                  sev_before=dict(sev_b),size_before_mb=size_b,cves_before=cves_b,base_before=orig_base)
            ...
            if ok2:
                ...
                rmi(tag)
            log.info(f"    → {total_b}→{r.vulns_after_total} ({r.reduction_pct:.1f}%)")
            results.append(r)
        ...
        if "copacetic" in self.strategies and (name,sn) not in self.done:
            ...
            if code != 0:
                ...
                log.info(f"    → Copa FAILED: {err2[:100]}")
            else:
                # Copa succeeded — now RESCAN to verify
                sf = str(self.out/"scans"/f"{name}_copa.json")
                sd = trivy_scan(ptag, sf)
                if sd is None:
                    # FIXED: Trivy re-scan failed → mark as FAILURE, not fake 100%
                    r.build_success=False; r.error_category="COPA_RESCAN_FAILED"
                    r.vulns_after_total=total_b; r.sev_after=dict(sev_b); r.cves_after=cves_b
                    r.notes="Copa patched but Trivy re-scan failed"
                    log.info(f"    → Copa patched but RESCAN FAILED (not counted as success)")
                else:
        ...
        if "scout" in self.strategies and (name,sn) not in self.done:
            ...
            if code == 0:
                try:
                    jd = json.loads(out2)
                    # Navigate Scout JSON structure
                    for key in ["recommendations","current","target"]:
                        if isinstance(jd, dict) and key in jd:
                            jd = jd[key]
                    if isinstance(jd, dict) and "tag" in jd:
                        rec_tag = jd["tag"]
                    elif isinstance(jd, list) and jd:
                        rec_tag = jd[0].get("tag") or jd[0].get("name")
                except: pass
            ...
            if not rec_tag:
                ...
                if code2 == 0:
                    for line in out2.splitlines():
                        # Match patterns like: "Tag 3.18 -> 3.21" or "Recommended tag: 3.21"
                        for pattern in [r'(\d+\.\d+[\w.-]*)\s*(?:→|->)\s*(\d+\.\d+[\w.-]*)',
                                        r'[Rr]ecommended.*?:\s*(\S+)',
                                        r'[Tt]ag.*?:\s*(\S+)']:
                            m = re.search(pattern, line)
                            if m:
                                rec_tag = m.group(m.lastindex)
                                break
                        if rec_tag: break
        ...
        if "autopatch" in self.strategies and (name,sn) not in self.done:
            ...
            r = R(image_name=name,strategy=sn,vulns_before_total=total_b,
                  ...
                  base_before=orig_base,base_after=new_base,notes=f"os={os_fam}")
            ...
            if ok2:
                ...
                rmi(tag)
            log.info(f"    → {total_b}→{r.vulns_after_total} ({r.reduction_pct:.1f}%), accept={r.acceptance}")
            results.append(r)
    ...
    def save(self):
        ...
        if self.results:
            flds = ["image_name","strategy","build_success","build_time_sec","error_category",
                    ...
                    "size_before_mb","size_after_mb","acceptance","base_before","base_after","notes"]
...
def main():
    ...
    p.add_argument("--copa-buildkit", default="", help="Copa buildkit container name")
    a = p.parse_args()
...
chmod +x "${WORK_DIR}/run_experiment_v2.py"
ok "Experiment v2 script written"

# ── Run ──────────────────────────────────────────────────────────────
step "Starting experiment v2 at $(date)..."
export PROGRESS_FILE

EXTRA=""
if [ "$MAX_IMAGES" -gt 0 ]; then EXTRA="--max-images $MAX_IMAGES"; fi

python3 "${WORK_DIR}/run_experiment_v2.py" \
    --dockerfile-dir "${REPO_DIR}/dockerfiles/" \
    --output-dir "$RESULTS_DIR" \
    --strategies $STRATEGIES \
    --copa-buildkit "$COPA_BK" \
    $EXTRA \
    2>&1 | tee -a "${WORK_DIR}/experiment_v2_output.log"

ok "Experiment v2 finished at $(date)"
...
cat > "${WORK_DIR}/stats_v2.py" << 'STATS_EOF'
#!/usr/bin/env python3
...
def main(rf,od,fd):
    ...
    for strat in ORDER:
        ...
        # Only count VR for images that had vulns to begin with
    ...
    # ── FIGURES ──────────────────────────────────────────────────────
    # Fig 2: top 6 most-vulnerable images
    ...
    if legacy:
        ...
        for si,strat in enumerate(ORDER):
            ...
            for j,fl in enumerate(fails):
                if fl: ax.text(bars[j].get_x()+bars[j].get_width()/2,bars[j].get_height()+2,
                              'F',ha='center',fontsize=7,fontweight='bold',color='red')
    ...
    # ── PRINT SUMMARY ────────────────────────────────────────────────
    print("\n"+"="*70)
    print("  EXPERIMENT v2 RESULTS — NUMBERS FOR THE PAPER")
    print("="*70)
    for strat in ORDER:
        s=ss.get(strat,{})
        print(f"\n  {strat}:")
        print(f"    Total images:    {s.get('total',0)}")
...
if __name__=="__main__":
    main(sys.argv[1], sys.argv[2], sys.argv[3] if len(sys.argv)>3 else str(Path(sys.argv[1]).parent.parent/"figures_v2"))
STATS_EOF

python3 "${WORK_DIR}/stats_v2.py" "$RESULTS_DIR/results.json" "$RESULTS_DIR" "$FIGURES_DIR"
...
echo ""
echo -e "${GREEN}════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}  EXPERIMENT v2 COMPLETE${NC}"
echo -e "${GREEN}════════════════════════════════════════════════════════════${NC}"
echo ""
