...
class SupplyChainResult:
    ...
    # False when the scan aborted partway. A partial scan that found
    # nothing is NOT a clean scan, and "SAFE" printed off an aborted
    # run is the most misleading output this module can produce.
    completed: bool = True
    # Checks that raised rather than returning a verdict, e.g. registry
    # signature verification with no network. Their absence from
    # `findings` means "not checked", not "nothing found".
    checks_errored: List[str] = field(default_factory=list)
...
def _extract_container_fs(image_name: str, extract_dir: str) -> bool:
    ...
    """
    container_name = f"autopatch_extract_{os.getpid()}_{uuid.uuid4().hex[:8]}"
...
def _extract_install_script_packages(lockdata: dict) -> Dict[str, str]:
    ...
    for pkg_path, pkg_info in packages.items():
        ...
        if pkg_info.get("hasInstallScript"):
            # Extract package name from path: pick the segment that
            # follows the LAST `node_modules/` boundary. Nested deps
            # of the form "node_modules/foo/node_modules/bar" must
            # resolve to "bar", not "foo/bar".
            if "node_modules/" in pkg_path:
                name = pkg_path.rsplit("node_modules/", 1)[1]
            else:
                name = pkg_path
            # Preserve scoped package names like "@scope/name"; trim
            # any trailing path inside the package directory.
...
def _verify_npm_signatures(package_dir: str, result: SupplyChainResult) -> None:
    ...
    try:
        ...
        # Registry signature verification did not run. Logged at DEBUG
        # and recorded nowhere, this presented as "provenance check
        # passed" in every report, which is the inverse of the truth.
        result.checks_errored.append("provenance:npm-signatures")
        logger.warning(
            "npm audit signatures did not complete (%s: %s); registry "
            "signature provenance for this image is UNVERIFIED",
            type(e).__name__, e,
        )
...
def _get_npm_publish_date(
    ...
    Uses the abbreviated package metadata endpoint
    (``https://registry.npmjs.org/<name>``) which returns a
    ``time`` map keyed by version. The previous implementation called
    the version-specific endpoint first, which almost never returns
    a top-level ``time`` field, then fell back to the abbreviated
    endpoint anyway, effectively doubling the request volume against
    the npm registry rate limit.
    """
    ...
    try:
        ...
        publish_time = time_map.get(version) if isinstance(time_map, dict) else None
...
def scan_supply_chain(
    ...
    try:
        ...
        # The partially-filled result was returned and consumed as a
        # completed scan, so an abort during check 2 reported checks
        # 3-6 as having found nothing.
        result.completed = False
        result.checks_errored.append(f"aborted:{type(e).__name__}")
        logger.error(
            "Supply chain scan ABORTED after checks %s (%s: %s); the "
            "remaining checks did not run and their absence from the "
            "findings must not be read as a clean result.",
            result.checks_run or "[none]", type(e).__name__, e,
        )
    finally:
