...
def _fetch_json_via_curl(url: str, timeout: int = 15) -> Optional[Any]:
    ...
    Prefers ``requests`` (already a runtime dependency) so we get a
    single, certificate-pinned HTTP stack across the project. Falls
    back to shelling out to ``curl`` only when ``requests`` is not
    importable (extreme stripped environments). The function name is
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.warning(
                "HTTP fetch failed for %s: status %d", url, resp.status_code,
            )
            return None
        data = resp.json()
        ...
        # requests not installed -- fall through to curl.
        pass
    except (ValueError, Exception) as e:  # requests.exceptions.* + JSON
        logger.warning(f"Invalid JSON or HTTP error from {url}: {e}")
        return None

    # Fallback: curl subprocess for environments without `requests`.
    cmd = [
        "curl", "-sfL",
...
# Fallback EOL upgrade table -- used ONLY when endoflife.date is
# unreachable (air-gapped CI, network outage). Loaded lazily from the
# YAML file shipped alongside this module so the table can be updated
# without code changes. A small hardcoded backstop covers the case
# where the YAML file is missing entirely (e.g. minimal install).
_FALLBACK_EOL_UPGRADES_BACKSTOP: Dict[str, Dict[str, str]] = {
    "python": {"2.7": "3.13", "3.6": "3.13", "3.7": "3.13", "3.8": "3.13",
               "3.9": "3.13"},
    "node": {"12": "22", "14": "22", "16": "22", "18": "22", "20": "22"},
    "golang": {"1.19": "1.23", "1.20": "1.23", "1.21": "1.23", "1.22": "1.23"},
    "ruby": {"2.6": "3.3", "2.7": "3.3", "3.0": "3.3", "3.1": "3.3"},
    "php": {"7.2": "8.3", "7.3": "8.3", "7.4": "8.3", "8.0": "8.3",
            "8.1": "8.3"},
    "openjdk": {"7": "21", "8": "21", "11": "21"},
    "rust": {"1.70": "1.85"},
    "dotnet": {"3.1": "8.0", "5.0": "8.0", "6.0": "8.0", "7.0": "8.0"},
    "perl": {"5.32": "5.40", "5.34": "5.40"},
    "erlang": {"24": "27", "25": "27"},
    "elixir": {"1.14": "1.18", "1.15": "1.18"},
}


def _load_eol_fallback() -> Dict[str, Dict[str, str]]:
    """Read the YAML EOL fallback table next to this module."""
    try:
        import yaml  # type: ignore
    except ImportError:
        logger.debug("PyYAML not available; using hardcoded EOL fallback.")
        return dict(_FALLBACK_EOL_UPGRADES_BACKSTOP)

    yaml_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "eol_fallback.yaml"
    )
    if not os.path.isfile(yaml_path):
        logger.debug(
            "eol_fallback.yaml not found at %s; using hardcoded backstop.",
            yaml_path,
        )
        return dict(_FALLBACK_EOL_UPGRADES_BACKSTOP)
    ...
    try:
        ...
        logger.warning(
            "Failed to load %s (%s: %s); using hardcoded backstop.",
    ...
    if isinstance(data, dict):
        for lang, entries in data.items():
            if isinstance(entries, dict):
                out[str(lang)] = {str(k): str(v) for k, v in entries.items()}
    if not out:
        return dict(_FALLBACK_EOL_UPGRADES_BACKSTOP)
    return out


_FALLBACK_EOL_UPGRADES: Dict[str, Dict[str, str]] = _load_eol_fallback()
