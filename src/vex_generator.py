...
def _resolve_subject(image_ref: str) -> Tuple[str, str]:
    """Split ``image_ref`` into an in-toto subject name and sha256 digest.

    The previous inline expression was
    ``image_ref.split("sha256:")[-1] if "sha256:" in image_ref else ""``,
    and ``main.py`` passes ``local_patched``, a TAG. Every attestation
    this pipeline produced therefore carried ``"sha256": ""``. An
    in-toto subject with an empty digest binds the predicate to
    nothing: a verifier cannot tell which image the remediation claim
    applies to, and any image can be presented as the subject. That
    voids the entire provenance chain, so a tag is resolved to a real
    digest here rather than silently degraded to "".

    Resolution order:
      1. ``repo@sha256:<hex>`` or a bare ``sha256:<hex>`` in the ref.
      2. ``docker inspect`` on the local daemon (RepoDigests, then .Id).
      3. Raise. A subject we cannot name is not one we may attest to.

    Returns:
        ``(name, digest_hex)`` where ``name`` has the digest stripped so
        it is a plain image reference, per the in-toto subject schema.
    """
    ref = (image_ref or "").strip()
    if not ref:
        raise ValueError("image_ref is empty; cannot build an attestation subject")
    ...
    # A tag. Ask the daemon what it actually resolves to.
    from .builder import get_image_digest, get_image_digest_local
    for resolver in (get_image_digest, get_image_digest_local):
        try:
            resolved = resolver(ref)
        except Exception:  # daemon unreachable, image absent
            resolved = None
        if not resolved:
            continue
        m = re.search(r"sha256:([a-f0-9]{64})", resolved)
        if m:
            return ref, m.group(1)

    raise ValueError(
        f"Cannot resolve {ref!r} to a sha256 digest. An in-toto subject "
        "without a digest binds the attestation to no image at all. "
        "Pass a digest reference, or ensure the image exists locally."
    )
...
def generate_remediation_attestation(
    ...
    subject_name, subject_digest = _resolve_subject(image_ref)
