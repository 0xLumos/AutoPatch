...
from .constants import DGA_ENTROPY_THRESHOLD
...
class NetworkProfile:
    ...
    # Packets the parsers could not decode. Every detector below scores
    # risk from what SURVIVED parsing, so a capture in which most
    # packets failed to parse yields a low score for the wrong reason.
    # Malformed framing is also exactly what an evasive implant
    # produces, which makes silently dropping these the worst possible
    # default. Counted here so the risk score can be qualified.
    http_parse_failures: int = 0
    dns_parse_failures: int = 0

    @property
    def parse_failures(self) -> int:
        return self.http_parse_failures + self.dns_parse_failures
...
def _try_parse_http(data: bytes, timestamp: float, host: str, profile: NetworkProfile) -> None:
    ...
    try:
        ...
        # Counted, not discarded: see NetworkProfile.http_parse_failures.
        profile.http_parse_failures += 1
