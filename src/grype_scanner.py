...
def scan_image(
    ...
    Args:
        ...
        skip_db_update: Pin the vulnerability database to whatever
            snapshot is already on disk. This mirrors Trivy's
            ``--skip-db-update`` and exists for the same reason: the
            after-scan must see exactly the advisory set the
            before-scan saw. Grype auto-updates its DB by default, so
            without this an advisory published between the two scans
            appears as a "new" vulnerability introduced by the patch,
            or a withdrawn one as a spurious fix. Implemented via
            ``GRYPE_DB_AUTO_UPDATE=false`` because grype exposes no
    ...
    env_override = {"GRYPE_DB_AUTO_UPDATE": "false"} if skip_db_update else None

    start_time = time.time()
    code, output = run_cmd(
        cmd, timeout=timeout, retries=retries, env_override=env_override
    )
