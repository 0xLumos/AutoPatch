...
def create_remediation_branch(
    ...
    # Refuse to operate on a dirty tree. ``git checkout -b <new> <base>``
    # silently carries any uncommitted working-tree changes onto the
    # new branch, which means the PR can end up containing unrelated
    # local edits the operator never reviewed. Detect with a strict
    # status check; abort with an actionable message.
    code, status = run_cmd(
        ["git", "-C", work_dir, "status", "--porcelain"]
    )
    if code != 0:
        raise PRCreationError(
            f"Could not inspect git status in {work_dir}: {status}"
        )
    if status.strip():
        raise PRCreationError(
            f"Refusing to create remediation branch on a dirty tree in "
            f"{work_dir}. Commit, stash, or discard local changes before "
            f"running with --create-pr. `git status --porcelain` returned:\n"
            f"{status.strip()}"
        )
...
def commit_changes(
    ...
    """
    # Stage files. A file that fails to stage is a file that will not
    # be in the commit, so continuing produced a partial commit that
    # looked successful: exactly the patched Dockerfile silently going
    # missing from the PR.
    for f in files:
...
def create_pull_request(
    ...
    # Extract PR URL from output. `gh` writes deprecation notices and
    # auth warnings to stdout alongside the link, so returning the raw
    # stdout handed callers a banner with a URL buried in it, which then
    # landed verbatim in the report and the CI annotation.
    pr_url = ""
    ...
    if not pr_url:
        raise PRCreationError(
            f"gh reported success but printed no pull-request URL; "
            f"cannot confirm the PR was created. Output:\n{output[:500]}"
        )
...
def create_remediation_pr(
    ...
    try:
        ...
        #
        # The return value used to be discarded, so a failed commit
        # still pushed the branch and opened a pull request. The result
        # was a PR containing no changes, reported to the operator as a
        # success with a URL: the pipeline claimed to have delivered a
        # remediation it had not committed.
        ...
        if not committed:
            raise PRCreationError(
                f"git commit failed on branch {branch}; refusing to open a "
                f"pull request with no changes in it."
            )
