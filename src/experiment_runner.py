...
from .utils import (
    run_cmd, save_json, save_csv, load_base_mapping,
...
def _fmt_pct(value: Optional[float]) -> str:
    """Format a possibly-undefined percentage for logging."""
...
class StrategyResult:
    ...
    # None when the baseline was zero and the ratio is undefined. It is
    # NOT 0.0: a zero would be averaged in as "this run achieved no
    # reduction", which is a different and false claim.
...
class ExperimentSummary:
    ...
    # Denominator and exclusions for the three statistics above. A mean
    # reported without its n, and without how many rows were dropped to
    # produce it, is not a reportable result.
    reduction_n: int = 0
    reduction_excluded_undefined: int = 0
    reduction_excluded_failed_build: int = 0
    reduction_regressions: int = 0
    min_reduction_pct: Optional[float] = None
    max_reduction_pct: Optional[float] = None
...
class ExperimentRunner:
    ...
    def run_strategy_scan_only(
        ...
        try:
            ...
            # Strategy A is the unpatched baseline: no remediation was
            # attempted, so the reduction is not defined rather than 0%.
            # Recording 0.0 put every baseline row into the mean and
            # dragged the reported average down by construction.
            result.reduction_percentage = None
    ...
    def run_strategy_naive(
        ...
        try:
            ...
            # Calculate reduction
            result.reduction_percentage = compute_reduction_percentage(
            ...
            logger.info(f"[Strategy B] Complete: {result.vulnerabilities_before} -> {result.vulnerabilities_after} vulns, {_fmt_pct(result.reduction_percentage)} reduction")
        except Exception as e:
    ...
    def run_strategy_autopatch(
        ...
        try:
            ...
            logger.info(f"[Strategy C] Complete: {result.vulnerabilities_before} -> {result.vulnerabilities_after} vulns, {_fmt_pct(result.reduction_percentage)} reduction, accepted={accepted}")
        except Exception as e:
    ...
    def run_strategy_copacetic(
        ...
        try:
            ...
            logger.info(
                ...
                f"{_fmt_pct(result.reduction_percentage)} reduction"
            )
        except Exception as e:
    ...
    def _compute_summary(self, results: List[StrategyResult]) -> ExperimentSummary:
        ...
        # Compute reduction statistics over successful builds.
        #
        # The previous filter was:
        #     r.build_success and r.reduction_percentage >= 0
        #
        # The `>= 0` clause selected on the OUTCOME. Every image the
        # patch made worse was deleted from the sample before the mean
        # was taken, so a run that regressed half the corpus reported
        # the same headline mean as one that regressed none of it. That
        # is survivorship bias in the primary reported metric, and it
        # inflates the number the paper quotes. Regressions are now
        # kept in the mean and counted separately; only rows where the
        # ratio is mathematically undefined (zero baseline) are
        # excluded, and that exclusion is reported.
        built = [r for r in results if r.build_success]
        summary.reduction_excluded_failed_build = len(results) - len(built)
        reductions = [
            r.reduction_percentage for r in built
            if r.reduction_percentage is not None
        ]
        summary.reduction_excluded_undefined = len(built) - len(reductions)
        summary.reduction_n = len(reductions)
        summary.reduction_regressions = sum(1 for x in reductions if x < 0)
        if reductions:
            summary.mean_reduction_pct = statistics.mean(reductions)
            summary.median_reduction_pct = statistics.median(reductions)
            summary.min_reduction_pct = min(reductions)
            summary.max_reduction_pct = max(reductions)
            if len(reductions) > 1:
                # Sample stdev (n-1). These are runs drawn from a corpus,
                # not the whole population of Dockerfiles.
                summary.std_dev_reduction_pct = statistics.stdev(reductions)
        logger.info(
            "Reduction stats: n=%d mean=%.2f%% median=%.2f%% sd=%.2f%% "
            "(regressions=%d, excluded: %d undefined baseline, %d failed build)",
            summary.reduction_n, summary.mean_reduction_pct,
            summary.median_reduction_pct, summary.std_dev_reduction_pct,
            summary.reduction_regressions,
            summary.reduction_excluded_undefined,
            summary.reduction_excluded_failed_build,
        )
