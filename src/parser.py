...
def _heredoc_terminator(line: str) -> Optional[str]:
    """Return the terminator word when ``line`` opens a BuildKit
    heredoc, else None.

    BuildKit supports ``RUN <<EOF``, ``COPY <<EOF /dest``, quoted forms
    (``<<'EOF'``, ``<<"EOF"``) and the indent-stripping ``<<-EOF``. The
    heredoc body is arbitrary text that the Dockerfile grammar does not
    own, so the parser must skip to the terminator rather than
    interpreting the payload as instructions.
    """
    stripped = line.strip()
    if "<<" not in stripped:
        return None
    # Only instructions can open a heredoc; a bare "<<" inside a
    # continuation is handled by the continuation tracker instead.
    first = stripped.split(None, 1)[0].upper() if stripped.split() else ""
    if first not in {"RUN", "COPY", "ADD"}:
        return None
    ...
    quoted, word = m.group(1), m.group(2)

    # `<<` is also the shell left-shift operator, so `RUN python -c
    # "print(1 << n)"` matches the pattern. Reading that as a heredoc
    # makes the parser swallow every following line until a line
    # equal to "n" appears, which never happens, so the runtime stage
    # of a multi-stage build silently disappears and is never
    # rewritten. Three checks separate a real heredoc from an
    # arithmetic shift.

    # 1. A quoted terminator (<<'EOF', <<"EOF") is unambiguous.
    if quoted:
        return word

    # 2. An unbalanced quote before the match means the `<<` is inside
    #    a string literal, so it is shell text and not a heredoc.
    prefix = stripped[:m.start()]
    if prefix.count('"') % 2 or prefix.count("'") % 2:
        return None

    # 3. Heredoc terminators are conventionally upper-case tokens
    #    (EOF, END, SCRIPT). A shift operand is a lower-case variable
    #    or a digit. Requiring the conventional shape costs the rare
    #    lower-case terminator, which then falls back to being parsed
    #    as ordinary lines, and buys immunity from every shift
    #    expression.
...
def parse_dockerfile_stages(
    ...
    # Lines that are logically inside another instruction are not
    # candidate FROM lines even when they begin with the token "from".
    # Two cases must be tracked, because both routinely carry text the
    # Dockerfile grammar does not own:
    #   * continuation: the previous logical line ended with a backslash
    #     ("RUN pip install \" followed by "fromage");
    #   * heredoc body: a BuildKit heredoc ("RUN cat <<EOF") whose
    #     payload is arbitrary text, very often Python containing
    #     "from os import path".
    # Treating either as a FROM closes the current stage and silently
    # discards every following line from the reconstructed Dockerfile.
    ...
    for i, line in enumerate(lines):
        ...
        # A line ending in an unescaped backslash continues the next.
        in_continuation = stripped.endswith("\\")

        opened = _heredoc_terminator(line)
        if opened:
            heredoc_terminator = opened
            continue

        if was_continuation:
            # Inside a multi-line instruction: never a FROM.
            continue

        # Cheap pre-filter, then full validation. The order matters: a
        # line must be proven to be a real FROM instruction *before*
        # the current stage is finalized.
        if not stripped or stripped[0] == "#":
            continue
        if not stripped.lower().startswith("from"):
            continue

        # Parse and validate before mutating any state. The first
        # whitespace-delimited token must be exactly "from"; startswith
        # alone matches "fromage", "from_cache", and similar.
        line_no_comment, comment = _split_from_line(line)
        parts = line_no_comment.split()
        if len(parts) < 2 or parts[0].lower() != "from":
            continue

        # Confirmed a real FROM: now finalize the previous stage.
        ...
        # Determine if base refers to a previous stage. C7 fix: Docker stage
        # names are case-insensitive, so "FROM builder" must resolve to a
        # stage declared "AS Builder". known_aliases stores lowercased names.
        is_stage_alias = resolved_image_ref.lower() in known_aliases
        ...
        # Track this stage's alias for future references (lowercased so
        # case-insensitive references resolve correctly, see C7 fix above).
        if alias_name:
            known_aliases.add(alias_name.lower())
...
def extract_preamble(dockerfile_text: str) -> List[str]:
    """Return the lines that precede the first FROM instruction.

    Everything above the first FROM is global Dockerfile state that a
    rewriter must preserve verbatim:

      * parser directives (``# syntax=docker/dockerfile:1``), which are
        only honoured on the first line of the file;
      * global ``ARG`` declarations, which are the only ARGs visible to
        a ``FROM`` line and are therefore exactly the values that
        ``FROM ${BASE_IMAGE}`` depends on;
      * ``ESCAPE`` directives and licence headers.

    :func:`parse_dockerfile_stages` starts recording at the first FROM,
    so a caller that reconstructs a Dockerfile purely from stages drops
    all of the above. Emitting a rewritten file without the ARG that its
    own FROM line references produces a Dockerfile that cannot build.
    """
    lines = dockerfile_text.splitlines()
    for i, line in enumerate(lines):
        stripped = line.strip()
        if not stripped or stripped[0] == "#":
            # A comment before the first FROM is preamble, but a syntax
            # directive must be kept in place, so keep scanning.
            continue
        parts = _split_from_line(line)[0].split()
        if parts and parts[0].lower() == "from":
            return lines[:i]
    # No FROM at all: the whole file is preamble.
    return list(lines)
...
def analyze_run_commands(dockerfile_text: str) -> List[RunCommand]:
    ...
    while i < len(lines):
        ...
        # C5 fix: bound the loop on i < len(lines). A Dockerfile whose final
        # line ends with a trailing backslash used to push i past the end and
        # re-evaluate _is_continuation_line(lines[i]), raising IndexError and
        # killing the whole run on an otherwise buildable file.
...
def detect_package_manager_from_dockerfile(dockerfile_text: str) -> Optional[str]:
    ...
    return None  # no system package manager detected
